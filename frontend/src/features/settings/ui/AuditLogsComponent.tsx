...
export function AuditLogsComponent({ scrollContainerRef: externalScrollRef }: Props) {
  ...
  const loadAuditLogs = async (isInitialLoad = false) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : t('settings.auditLogsFailed');
  ...
  const columns: ColumnsType<AuditLog> = [
    ...
    {
      title: t('settings.columnMessage'),
      dataIndex: 'message',
      key: 'message',
    ...
    {
      title: t('settings.columnWorkspace'),
    ...
    {
      title: t('settings.columnCreated'),
      dataIndex: 'createdAt',
      key: 'createdAt',
      width: 250,
  ...
  return (
    <div className="max-w-[1200px]">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-xl font-bold dark:text-white">{t('settings.auditLogsTitle')}</h2>
        <div className="text-sm text-gray-500 dark:text-gray-400">
          {isLoading ? (
            <Spin indicator={<LoadingOutlined spin />} />
          ) : (
            t('settings.logsCount', { count: auditLogs.length, total })
          )}
        </div>
      </div>
        ...
        <>
          ...
          {!hasMore && auditLogs.length > 0 && (
            <div className="py-4 text-center text-sm text-gray-500 dark:text-gray-400">
              {t('settings.allLogsLoaded', { total: auditLogs.length })}
            </div>
          )}
        </>
      )}
