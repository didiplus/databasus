...
export function SettingsComponent({ contentHeight }: Props) {
  ...
  const loadSettings = async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : t('settings.failedToLoad');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setHasChanges(false);
      message.success(t('settings.updatedSuccess'));
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : t('settings.failedToUpdate');
      message.error(errorMessage);
    } finally {
      setIsSaving(false);
    }
                  ...
                  <div className="flex items-start justify-between border-b border-gray-200 pb-4 dark:border-gray-700">
                    <div className="flex-1 pr-20">
                      ...
                      <div className="mt-1 text-gray-500 dark:text-gray-400">
                        {t('settings.externalRegistrationsHint')}
                      </div>
                    </div>
                  ...
                  {!formSettings.isAllowExternalRegistrations && (
                    <div className="flex items-start justify-between border-b border-gray-200 pb-4 dark:border-gray-700">
                      <div className="flex-1 pr-20">
                        ...
                        <div className="mt-1 text-gray-500 dark:text-gray-400">
                          {t('settings.memberInvitationsHint')}
                        </div>
                      </div>
              ...
              <div className="flex items-center rounded-md border border-gray-300 bg-gray-50 px-3 py-2 !font-mono text-sm text-gray-700 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200">
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    message.success(t('settings.endpointCopied'));
                  }}
                >
                  📋
                </Button>
