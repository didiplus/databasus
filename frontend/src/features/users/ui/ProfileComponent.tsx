...
export function ProfileComponent({ contentHeight }: Props) {
  ...
  const validatePasswordFields = (): boolean => {
    ...
    if (!newPassword) {
      ...
      setNewPasswordError(true);
        message.error(t('auth.passwordMin6'));
      isValid = false;
    } else {
    ...
    if (!confirmPassword) {
      ...
      setConfirmPasswordError(true);
        message.error(t('auth.newPasswordsDoNotMatch'));
      isValid = false;
    } else {
    ...
    try {
      ...
      if (user?.email) {
        try {
          ...
          message.success(t('auth.successfullySignedInNew'));
        } catch (signInError: unknown) {
          const errorMessage =
            signInError instanceof Error
              ? signInError.message
              : t('auth.failedSignInNew');
      ...
      const errorMessage = error instanceof Error ? error.message : t('auth.failedChangePassword');
      message.error(errorMessage);
    } finally {
  ...
  const handleProfileUpdate = async () => {
    ...
    try {
      ...
      if (Object.keys(request).length === 0) {
        message.info(t('profile.noChangesToSave'));
        setIsUpdatingProfile(false);
        return;
      }

      await userApi.updateUserInfo(request);
      message.success(t('profile.updatedSuccessfully'));

      // Reload user profile
      loadUserProfile();
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : t('profile.failedToUpdate');
      message.error(errorMessage);
    } finally {
      setIsUpdatingProfile(false);
    }
              ...
              <>
                <div className="mb-6">
                  ...
                  <div className="max-w-md">
                    <div className="text-xs font-semibold dark:text-gray-200">{t('profile.userId')}</div>
                    ...
                    <div className="mt-2 mb-1 text-xs font-semibold dark:text-gray-200">{t('common.email')}</div>
                    <Input
                      value={editEmail}
                      onChange={(e) => {
                        setEditEmailError(false);
                    ...
                    {user.email === 'admin' && (
                      <div className="mb-4 text-xs text-gray-500 dark:text-gray-400">
                        {t('profile.adminEmailCannotChange')}
                      </div>
                    )}

                    <div className="mt-2 mb-1 text-xs font-semibold dark:text-gray-200">{t('common.role')}</div>
                    <div className="mb-4">
