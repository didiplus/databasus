  ...
  const sendTestNotification = async () => {
    ...
    try {
      ...
      if (notifier.notifierType === NotifierType.SLACK) {
        alert(t('notifiers.slackChannelHint'));
      }
    }
    ...
    <div>
      {isShowName && (
        <div className="mb-1 flex w-full flex-col items-start sm:flex-row sm:items-center">
          <div className="mb-1 min-w-[150px] sm:mb-0">{t('common.name')}</div>
          ...
          <Input
            ...
            placeholder={t('notifiers.namePlaceholder')}
          />
        </div>
      )}

      <div className="mb-1 flex w-full flex-col items-start sm:flex-row sm:items-center">
        <div className="mb-1 min-w-[150px] sm:mb-0">{t('common.type')}</div>

        <div className="flex items-center">
          <Select
            value={notifier?.notifierType}
            options={[
              { label: t('notifiers.typeTelegram'), value: NotifierType.TELEGRAM },
              { label: t('notifiers.typeEmail'), value: NotifierType.EMAIL },
              { label: t('notifiers.typeWebhook'), value: NotifierType.WEBHOOK },
              { label: t('notifiers.typeSlack'), value: NotifierType.SLACK },
